...
(function() {
  ...
  Workbook = (function() {
    ...
    Workbook.prototype.save = function(cb) {
      ...
      //var p = 'cd ' + target + '/; jar -cfM ../' + this.fname + ' .; cd ..;';

      var clearTarget = function(target,error,cb){
      ...
      if( platform != "darwin" && platform != "linux" ){
            ...
            var output = fs.createWriteStream(path.join(target, './../'+this.fname));
             
            output.on('close', function() {
              console.log(archive.pointer() + ' total bytes');
              console.log('archiver has been finalized and the output file descriptor has closed.');
              return clearTarget(target,null, cb);

            });

            archive.on('error', function(err) {
             return clearTarget(target,err, cb);
            });
    ...
    var target = path.join(path.resolve(this.fpath), this.id);
